...
class Auth:
    ...
    def create_email_token(self, data: dict):
        ...
        when it expires, and what user id it belongs to.
        
        :param self: Represent the instance of the class
        :param data: dict: Pass in the data that is to be encoded
        :return: A token that is encoded with the user's email, a timestamp of when it was created and an expiration date
        """
        to_encode = data.copy()
    ...
    async def get_email_from_token(self, token: str):
        ...
        The function first tries to decode the token using jwt.decode(). If successful, it will return the email address associated with that user's account.
        If unsuccessful, it will raise an HTTPException.
        
        :param self: Represent the instance of the class
        :param token: str: Pass in the token that was sent to the user's email address
        :return: The email address that is encoded in the jwt
        """
        try:
            payload = jwt.decode(token, self.SECRET_KEY, algorithms=[self.ALGORITHM])
            email = payload["sub"]
